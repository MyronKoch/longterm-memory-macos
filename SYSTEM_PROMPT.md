...
- **Cross-Device Sync**: Bidirectional M1↔M3 via iCloud + pg_dump
...
FROM entities WHERE name = '[YOUR_ENTITY_NAME]'
UNION ALL
...
## Specialized Tables (Schema Evolution)

The core schema (`entities`, `observations`, `insights`) handles most knowledge. However, when a **domain-specific pattern emerges repeatedly** with consistent structure, a specialized table may be warranted.

### When to Propose a New Table

All criteria must be met:

1. **Recurring Pattern**: You've captured 3+ observations on the same topic with similar structure
2. **Queryable Fields**: The knowledge has distinct, searchable attributes (not just prose)
3. **Lookup Value**: Future sessions will need to query this by specific fields, not just semantic search
4. **Won't Fit Tags**: The structure is too complex for `observations` + tags/metadata

### When NOT to Create a Table

- One-off insights (use `observations` with high importance)
- Simple categorization (use tags array)
- Flexible metadata (use JSONB `metadata` field)
- Fewer than 3 entries expected

### Process

1. **Propose first**: Tell the user "I'm seeing a pattern in [X] that might warrant its own table. The fields would be [Y]. Should I create it?"
2. **Design for queries**: Include fields that enable useful WHERE clauses
3. **Keep it small**: 5-8 columns max; avoid over-engineering
4. **Document the purpose**: Add a comment to the table

### Schema Template
```sql
-- [TABLE_NAME]: [One-line purpose]
-- Created: [DATE] | Reason: [Why observations weren't sufficient]
CREATE TABLE [table_name] (
    id SERIAL PRIMARY KEY,
    [domain]_date DATE NOT NULL,           -- when it happened
    title VARCHAR(255) NOT NULL,           -- quick identifier
    [key_field_1] TEXT,                    -- the main queryable insight
    [key_field_2] TEXT,                    -- supporting detail
    [key_field_3] TEXT,                    -- optional context
    created_at TIMESTAMP DEFAULT NOW()
);
```

### Existing Specialized Tables

| Table | Purpose | Fields |
|-------|---------|--------|
| `bash_compatibility_lessons` | Portable shell syntax | problematic_syntax, portable_solution, affected_files |
| `blockchain_mcp_breakthroughs` | MCP development patterns | key_insight, impact, technical_details, pattern_established |
| `mcp_factory_system` | Factory automation state | (check schema) |
